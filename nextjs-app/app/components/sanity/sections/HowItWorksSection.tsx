...
export default function HowItWorksSection({ block }: HowItWorksSectionProps) {
  ...
  return (
    <section className={`${backgroundColor} py-8 md:py-16`}>
        ...
        <div className="relative max-w-5xl mx-auto">
          ...
          {steps.length > 1 && (
            <>
              {Array.from({ length: steps.length - 1 }).map((_, index) => (
                <div
                  key={index}
                  className={`hidden md:block absolute left-[${33.33 * (index + 1)}%] top-1/4 -translate-x-1/2 -translate-y-1/2 z-10`}
                >
                  <ArrowSvg />
                </div>
              ))}
          ...
          <div className={`grid md:grid-cols-${steps.length} gap-8`}>
            {steps.map((step, index) => (
              <div key={index} className="text-center space-y-4 p-3">
